...
For 6_outreach_angle, value must be one sentence only, maximum 20 words.
It must reference exactly one specific gap found on the website.
Do not mention multiple issues. Make it sound observed, not templated.
...
"""
    ...
    analysis["6_outreach_angle"] = normalize_outreach_angle(analysis)
    return analysis


def normalize_outreach_angle(analysis: Dict[str, Any]) -> str:
    raw_angle = str(analysis.get("6_outreach_angle", "")).strip()
    weak_points = analysis.get("4_weak_points_and_gaps", [])

    # Prefer model output if it is already a short single sentence.
    if raw_angle and len(re.findall(r"[.!?]", raw_angle)) <= 1 and len(raw_angle.split()) <= 20:
        return raw_angle.rstrip(".!?") + "."

    gap_text = "your site has no clear primary CTA above the fold"
    if isinstance(weak_points, list) and weak_points:
        first_gap = weak_points[0]
        if isinstance(first_gap, dict):
            candidate = str(first_gap.get("finding", "")).strip()
            if candidate:
                gap_text = candidate

    words = re.findall(r"\b[\w'-]+\b", gap_text.lower())
    clipped_gap = " ".join(words[:14]) if words else "your site has no clear primary CTA"
    sentence = f"Noticed {clipped_gap}; we can help fix that quickly."
    final_words = sentence.split()[:20]
    final = " ".join(final_words).rstrip(".!?") + "."
    return final
